...
Xguess=T./1e13; Xguess(Xguess==0)=1e-30; TYPX=Xguess;
...
Xguess=T./100; TYPX=Xguess; TYPX=ones(size(T));
%logXguess=log10(T/10000); %Xguess(Xguess==0)=1e-30; 

[Xguess,masserr,J,RSI,C] = NRlogXnl_massbalancerrnosolid_NR(Xguess,ASOLUTION,KSOLUTION,ASOLID,KSOLID,T,TYPX,flag2);

%[Xguess,masserr,J,RSI,C] = nl_massbalancerrnosolid_NR(Xguess,ASOLUTION,KSOLUTION,ASOLID,KSOLID,T,TYPX)

% if max(abs(masserr))>0.001
%         Xguess=Xguess/1000000;
%         [Xguess,masserr,J,RSI,C] = NRlogXnl_massbalancerrnosolid_NR(Xguess,ASOLUTION,KSOLUTION,ASOLID,KSOLID,T,TYPX,flag2)
% end
...
%RSI(RSI>0)=min(T)/10; 
RSI(RSI>0)=0;
XguessN=[(Xguess); RSI]; TYPX=[Xguess; ones(size(RSI))];
